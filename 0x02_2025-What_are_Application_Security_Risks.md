...
<table>
  ...
  <tr>
   ...
   <td>
    <strong>セキュリティ対策の未実施の平均発生率（カバー率による重み付けあり）。</strong>
   </td>
   <td>
...
今回のイテレーションで使用した計算式は 2021 年版と同様ですが、次のように重み付けが若干変更されています: (最大発生率% * 1000) + (最大カバレッジ% * 100) + (平均悪用可能性 * 10) + (平均影響度 * 20) + (総発生件数 / 10000) = リスク スコア。
