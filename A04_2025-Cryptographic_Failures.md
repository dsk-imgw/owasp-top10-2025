...
* パスワードは、Argon2、yescrypt、scrypt、PBKDF2-HMAC-SHA-256 など、ワーク ファクター (遅延係数) を持つ強力な適応型ソルト付きハッシュ関数を使用して保存します。bcrypt を使用しているレガシー システムについては、[OWASP OWASP Cheat Sheet: Password Storage](https://cheatsheetseries.owasp.org/cheatsheets/Password_Storage_Cheat_Sheet.html) で詳細なアドバイスを参照できます。
...
* 高リスク システムが 2030 年末までに安全になるように、今すぐ耐量子暗号 (PQC) の準備をする必要があります（参考文献 (ENISA) を参照）。
